#!/usr/bin/env bash
# ADC PMS — Hostinger prep resume (steps 2-6; backup already exists)
set -euo pipefail
...
if [ -d "$BASE/app/.git" ]; then
  ...
  git clone --depth 1 $REPO "$BASE/app" && cd "$BASE/app"
fi
git log --oneline -1
...
if [ ! -f vendor/autoload.php ] || [ ! -d vendor/laravel ]; then
  ...
  echo "vendor present — skipping (will verify)"
