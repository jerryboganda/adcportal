    ...
    <div class="row mb-3">
        <div class="col-md-4">
            <div class="card"><div class="card-body py-3">
                ...
                <h4 class="mb-0">{{ currency_format($totals['billed']) }}</h4>
            </div></div>
        </div>
        <div class="col-md-4">
            <div class="card"><div class="card-body py-3">
                <small class="text-muted">{{ __('Collected') }}</small>
                <h4 class="mb-0 text-success">{{ currency_format($totals['paid']) }}</h4>
            </div></div>
        </div>
        <div class="col-md-4">
            <div class="card"><div class="card-body py-3">
                <small class="text-muted">{{ ('Outstanding') }}</small>
                <h4 class="mb-0 text-danger">{{ currency_format($totals['due']) }}</h4>
            </div></div>
        </div>
    </div>
            ...
            <table class="table table-hover align-middle">
                ...
                @forelse($invoices as $inv)
                    <tr>
                        ...
                        <td>{{ currency_format($inv->total) }}</td>
                        <td>{{ currency_format($inv->paid_total) }}</td>
                        <td class="{{ $inv->balance_due > 0 ? 'text-danger fw-bold' : 'text-muted' }}">{{ currency_format($inv->balance_due) }}</td>
                        <td>
                        ...
                        <td class="text-end">
                            <a href="{{ route('invoices.show', $inv) }}" class="btn btn-sm btn-outline-primary"><i class="ti ti-eye"></i></a>
                            <a href="{{ route('invoices.pdf', $inv) }}" class="btn btn-sm btn-outline-secondary"><i class="ti ti-file-download"></i></a>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="8" class="text-center text-muted py-5">{{ __('No invoices yet. Create one from a completed study.') }}</td></tr>
                @endforelse
                </tbody>
            </table>
