...
abstract class ApiTestCase extends TestCase
{
    ...
    /**
     * A registered patient. MRN is generated by the model (per-tenant unique).
     */
    protected function makePatient(
        Business $business,
        string $name = 'Reporting Test Patient',
        int $age = 40,
        string $gender = 'male',
        ?string $dob = null,
    ): \App\Models\Customer {
        // Mirror the booking/reception convention: a patient is a `customer`
        // user row plus the clinical Customer record linked through user_id.
        $user = User::create([
            'name' => $name,
            'email' => 'patient.'.md5($name.uniqid('', true)).'@patients.local',
        ...
        return \App\Models\Customer::create([
            ...
            'gender' => $gender,
            'age' => $age,
            'dob' => $dob ?? now()->subYears($age)->toDateString(),
            'business_id' => $business->id,
            'created_by' => $business->created_by,
        ]);
    }
    ...
    /**
     * A study already in the radiology pipeline (default: acquired, i.e. on a
     * reading worklist) without going through the whole booking→acquire HTTP
     * dance. For reporting tests the interesting behaviour starts here.
     *
     * @param array<string,mixed> $overrides
     */
    protected function makeStudy(Business $business, $service, $patient, array $overrides = []): \App\Models\Appointment
    {
        // `token_number` is allocated by StudyTokenAllocator (forceFill), never
        // mass-assigned from a request — mirror that here.
        $token = $overrides['token_number'] ?? null;
        unset($overrides['token_number']);

        $study = \App\Models\Appointment::create(array_merge([
        ...
        if ($token !== null) {
            $study->forceFill(['token_number' => $token])->save();
        }

        return $study;
    }

    /** A tenant service by code. */
    protected function tenantService(Business $business, string $code)
    {
        return \App\Models\Service::where('code', $code)->where('business_id', $business->id)->firstOrFail();
    }
