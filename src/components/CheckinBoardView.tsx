...
export const CheckinBoardView: React.FC<CheckinBoardViewProps> = ({
  ...
  // No token-slip / wristband state: both documents are rendered by the print
  // engine, which owns its own preview and its own paper geometry.
  ...
  return (
    <div className="space-y-6">
                    ...
                    <tr key={apt.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="py-2.5 px-3 text-right whitespace-nowrap">
                        <div className="flex items-center justify-end space-x-1">
                          ...
                          {/* Both of these open the print engine's own preview: the
                              document that goes on the thermal printer is the document
                              on screen, on the paper the registry says it belongs on. */}
                          <button
                            onClick={() =>
                              void openPrintPreview({ artifact: 'token', id: apt.id, paper: 'thermal80' })
                            }
                            title="Open the queue token slip"
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-200">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500">PolytronX - Enterprise PACS & RIS Cashier Counter</p>
                </div>
              </div>
              <button
                onClick={() => {
      ...
      {/*
        * The queue token slip and the wristband label used to be two hand-built
        * modals here, each with its own hardcoded "PolytronX" letterhead and its
        * own `window.print()`. Both printed the application shell instead of the
        * document, and neither could carry tenant branding.
        *
        * They are now documents of the print engine (`token`, `label`), opened in
        * the shared preview, on the paper the artifact registry assigns them.
        */}
      ...
      {showManifestModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-2xl max-w-4xl w-full p-6 shadow-2xl space-y-4 max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between pb-3 border-b border-slate-200">
              <div>
                <h3 className="font-bold text-slate-900 text-base">Daily Front Desk Manifest</h3>
              ...
              <div className="flex items-center space-x-2">
                {/*
                  * This list is a screen review, not the printed document. The
                  * manifest that goes on paper is the engine's `manifest` artifact:
                  * A4, tenant letterhead, one row per study, and — unlike the old
                  * `window.print()`, which printed the application shell — it
